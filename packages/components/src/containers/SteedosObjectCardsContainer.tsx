...
import {
  SteedosObjectCards,
  SteedosObjectCardsProps,
} from '../components/cards/SteedosObjectCards'
...
export interface SteedosObjectCardsContainerProps
  extends Omit<
    SteedosObjectCardsProps,
    | 'column'
    | 'errorMessage'
...
export const SteedosObjectCardsContainer = React.memo(
  (props: SteedosObjectCardsContainerProps) => {
    ...
    ) as SteedosObjectColumnSubscription | undefined
    ...
    const { allItems, filteredItems } = useColumnData<EnhancedSteedosObject>(
      columnId,
      { mergeSimilar: false },
    )

    const clearedAt = column && column.filters && column.filters.clearedAt
    const olderDate = getOlderSteedosObjectDate(allItems)
...
SteedosObjectCardsContainer.displayName = 'SteedosObjectCardsContainer'
